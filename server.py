...
cookie = SessionCookie(
    cookie_name=configs.Coockie['cookie_name'],
    identifier=configs.Coockie['identifier'],
    auto_error=configs.Coockie['auto_error'],
    secret_key=configs.Coockie['secret_key'], 
...
def detect_single(img_src='dataset/images/test',user_id=0,save_img=True,PCB_Id=0):
    ...
    # 保存目录创建
    os.makedirs(configs.Image_detect_path, exist_ok=True) 
    os.makedirs(configs.origin_Image_path, exist_ok=True) 
    os.makedirs(configs.Text_path, exist_ok=True)
    save_dir = Path(configs.Image_detect_path)  # 增量创建保存目录
    # 数据加载
